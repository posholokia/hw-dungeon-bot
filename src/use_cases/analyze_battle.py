...
# )
# from models.dto import State, TitalRoles, Titan, TitanElements
...
#             cx = (left[0] + right[0]) // 2
#             cy = (left[1] + right[1]) // 2
#             sample_points = [(cx + dx, cy + dy) for dx, dy in self._dead_sample_offsets]
...
#         health = self._bar_percent(
#             cx, cy, self._titans.health_offset_y, _is_health_green
#         )
#         energy = self._bar_percent(
#             cx, cy, self._titans.energy_offset_y, _is_energy_yellow
#         )
#         return Titan(
#             name=entry.name,
#             health_prc=health,
